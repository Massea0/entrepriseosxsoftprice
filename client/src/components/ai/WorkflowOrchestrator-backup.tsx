...
const WorkflowOrchestrator: React.FC = () => {
  ...
  const handleWebSocketMessage = (data: unknown) => {
    switch (data.type) {
      ...
      case 'workflow_status_update':
        setActiveExecutions(data.data.activeExecutions);
        setExecutionStats(data.data.stats);
        break;
      case 'workflow_execution_started':
        setActiveExecutions(prev => [data.execution, ...prev]);
        toast({
          title: "Workflow démarré",
          description: `Exécution ${data.execution.id} en cours`,
          variant: "default"
        });
        break;
  ...
  const loadWorkflows = async () => {
    ...
    try {
      const response = await fetch('/api/ai-workflow-orchestrator?action=get_workflows', {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        console.log('✅ Workflows chargés:', data.workflows.length);
      } else {
  ...
  const loadExecutions = async () => {
    try {
      const response = await fetch('/api/ai-workflow-orchestrator?action=get_executions', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const executeWorkflow = async (workflowId: string, initialData?: unknown) => {
    try {
      const response = await fetch('/api/ai-workflow-orchestrator', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
