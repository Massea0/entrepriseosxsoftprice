-- =====================================================
-- MIGRATION COMPLÈTE ENTERPRISE OS (VERSION CORRIGÉE)
-- Date: 2025-07-29
-- Description: Migration complète sans toucher au schéma auth
-- =====================================================
...
-- =====================================================
-- FUNCTIONS (Sans toucher au schéma auth)
-- =====================================================
...
-- Function to get user company_id from JWT
...
-- =====================================================
-- RLS POLICIES - VERSION SIMPLIFIÉE
-- =====================================================

-- Pour le moment, permettre l'accès aux utilisateurs authentifiés
-- Les politiques plus complexes seront ajoutées après

-- Companies: Tous les utilisateurs authentifiés peuvent lire
...
-- Projects: Lecture pour tous les authentifiés
CREATE POLICY "projects_authenticated_read" ON projects
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- Tasks: Lecture pour tous les authentifiés
CREATE POLICY "tasks_authenticated_read" ON tasks
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- Invoices: Lecture pour tous les authentifiés (temporaire)
CREATE POLICY "invoices_authenticated_read" ON invoices
  FOR SELECT USING (auth.uid() IS NOT NULL);

-- Devis: Lecture pour tous les authentifiés (temporaire)
CREATE POLICY "devis_authenticated_read" ON devis
  FOR SELECT USING (auth.uid() IS NOT NULL);
